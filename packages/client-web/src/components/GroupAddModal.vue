      ...
      <div class="p-4 space-y-4">
        ...
        <div class="space-y-2">
          ...
          <div class="bg-gray-900 border rounded-md border-gray-750">
            <input
              class="w-full px-4 py-2 -mt-px text-gray-400 bg-transparent border rounded-sm border-gray-750 focus:outline-none focus:border-gray-650"
              type="text"
...
export default {
  ...
  methods: {
    updateSearch() {
      //TODO: implement frined searching in modals.
    },
    async addUsers() {
      try {
        for (const friend of this.friends.filter((f) => f.selected)) {
          await this.$store.dispatch("groupAdd", {
